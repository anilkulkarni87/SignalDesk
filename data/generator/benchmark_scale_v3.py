...
def benchmark_one(args, customer_count: int) -> Dict:
    ...
    if structural_report.exists():
        ...
        if "campaign_quality" in structural:
            result["campaign_open_rate_pct"] = structural["campaign_quality"]["open_rate_pct"]
            result["campaign_click_rate_pct"] = structural["campaign_quality"]["click_rate_pct"]
            result["consent_opt_out_violations"] = structural["campaign_quality"]["effective_opt_out_violations"]
        if "consent_quality" in structural:
            result["consent_rows_per_customer"] = structural["consent_quality"]["actual_rows_per_customer"]
    ...
    if semantic_report.exists():
        ...
        if "campaign_engagement" in tests:
            result["campaign_price_sensitive_click_lift_points"] = tests["campaign_engagement"].get(
                "price_sensitive_click_lift_percentage_points"
            )
        if "subscription_behavior" in tests:
            result["stable_active_subscription_rate_pct"] = tests["subscription_behavior"].get(
                "stable_active_subscription_rate_pct"
            )
            result["dormant_active_subscription_rate_pct"] = tests["subscription_behavior"].get(
                "dormant_active_subscription_rate_pct"
            )
...
def write_csv(path: Path, results: List[Dict]):
    scalar_keys = [
        ...
        "campaign_open_rate_pct",
        "campaign_click_rate_pct",
        "consent_opt_out_violations",
        "consent_rows_per_customer",
        "campaign_price_sensitive_click_lift_points",
        "stable_active_subscription_rate_pct",
        "dormant_active_subscription_rate_pct",
    ]
...
# python data/generator/benchmark_scale_v3.py \
#   --generator data/generator/generate_synthetic_cdp_v5.py \
#   --structural-validator data/generator/validate_synthetic_cdp_v2.py \
#   --semantic-validator data/generator/validate_semantic_behavior_v2.py \
#   --sizes 1000 10000 100000 \
#   --products 1000 \
#   --campaigns 60 \
#   --avg-orders 5 \
