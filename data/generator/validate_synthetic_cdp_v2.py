...
ID_CONTRACTS = {
    ...
    "campaign_id": ("CAM", 6, "campaigns"),
    "exposure_id": ("EXP", 10, "campaign_exposures"),
    "subscription_id": ("SUB", 9, "subscriptions"),
    "consent_id": ("CN", 9, "consent_preferences"),
    "order_id": ("O", 9, "orders"),
...
def validate_consents(input_dir: Path, counts: Dict[str, int]):
    rows = invalid_pk = invalid_customer = invalid_channel = 0
    previous = 0
    effective = {}

    for row in read_rows(input_dir / "consent_preferences.csv"):
        rows += 1
        current = parse_generated_id(row["consent_id"], "CN", 9)
        if current is None or current <= previous:
            invalid_pk += 1
        if current is not None:
            previous = current
        ...
        if row["channel"] not in {"EMAIL", "SMS", "PUSH"}:
            invalid_channel += 1

        effective[(row["customer_id"], row["channel"])] = (
            row["status"],
    ...
    pk = {
        "rows": rows,
        "expected_rows": counts["consent_preferences"],
        "malformed_or_non_monotonic": invalid_pk,
        "passed": rows == counts["consent_preferences"] and invalid_pk == 0,
    }
    fk = {
        "consent_preferences.customer_id": {
    ...
    quality = {
        ...
        "expected_rows_per_customer": 3,
        "actual_rows_per_customer": round(rows / counts["customers"], 3)
        if counts["customers"] else 0,
        "passed": invalid_channel == 0 and rows == counts["customers"] * 3,
    }
    return pk, fk, quality, effective


def validate_subscriptions(input_dir: Path, counts: Dict[str, int]):
    rows = invalid_pk = invalid_customer = 0
    previous = 0

    for row in read_rows(input_dir / "subscriptions.csv"):
        rows += 1
        current = parse_generated_id(row["subscription_id"], "SUB", 9)
        if current is None or current <= previous:
            invalid_pk += 1
        if current is not None:
            previous = current
    ...
    pk = {
        "rows": rows,
        "expected_rows": counts["subscriptions"],
        "malformed_or_non_monotonic": invalid_pk,
        "passed": rows == counts["subscriptions"] and invalid_pk == 0,
    }
    fk = {
...
def validate_campaign_exposures(
    input_dir: Path,
    counts: Dict[str, int],
    effective_consents,
):
    rows = invalid_pk = invalid_customer = invalid_campaign = invalid_order = 0
    consent_violations = invalid_channel = 0
    previous = 0
    delivered = opened = clicked = 0

    for row in read_rows(input_dir / "campaign_exposures.csv"):
        rows += 1
        current = parse_generated_id(row["exposure_id"], "EXP", 10)
        if current is None or current <= previous:
            invalid_pk += 1
        if current is not None:
            previous = current
        ...
        if not valid_ref(row["customer_id"], "customer_id", counts["customers"]):
            invalid_customer += 1
        if not valid_ref(row["campaign_id"], "campaign_id", counts["campaigns"]):
            invalid_campaign += 1
        if row["converted_order_id"] and not valid_ref(
            row["converted_order_id"], "order_id", counts["orders"]
        ):
            invalid_order += 1
        ...
        consent = effective_consents.get((row["customer_id"], channel))
        sent_at = parse_dt(row["sent_at"])
        if consent and sent_at:
            status, updated_at = consent
            if status == "OPTED_OUT" and updated_at and updated_at <= sent_at:
    ...
    pk = {
        "rows": rows,
        "expected_rows": counts["campaign_exposures"],
        "malformed_or_non_monotonic": invalid_pk,
        "passed": rows == counts["campaign_exposures"] and invalid_pk == 0,
    }
    fk = {
        ...
        "campaign_exposures.campaign_id": {
            "invalid_refs": invalid_campaign,
            "passed": invalid_campaign == 0,
        },
        "campaign_exposures.converted_order_id": {
    ...
    quality = {
        ...
        "invalid_channels": invalid_channel,
        "effective_opt_out_violations": consent_violations,
        "passed": invalid_channel == 0 and consent_violations == 0,
    }
...
def main():
    ...
    primary_keys["campaigns.csv"] = validate_simple_pk(
        args.input_dir / "campaigns.csv", "campaign_id", counts["campaigns"]
    )

    consent_pk, consent_fks, consent_quality, effective_consents = validate_consents(
        args.input_dir, counts
    )
    primary_keys["consent_preferences.csv"] = consent_pk
    foreign_keys.update(consent_fks)

    subscription_pk, subscription_fks = validate_subscriptions(args.input_dir, counts)
    primary_keys["subscriptions.csv"] = subscription_pk
    foreign_keys.update(subscription_fks)
    ...
    exposure_pk, exposure_fks, campaign_quality = validate_campaign_exposures(
        args.input_dir,
        counts,
        effective_consents,
    )
    primary_keys["campaign_exposures.csv"] = exposure_pk
    foreign_keys.update(exposure_fks)
    ...
    passed = (
        ...
        and consent_quality["passed"]
        and campaign_quality["passed"]
    )
