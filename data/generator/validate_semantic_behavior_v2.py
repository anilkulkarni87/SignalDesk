...
def build_features(input_dir: Path, truth: Dict[str, str]) -> Dict[str, dict]:
    ...
    for row in read_rows(input_dir / "campaign_exposures.csv"):
        ...
        if row["delivery_status"] == "DELIVERED":
            features[customer_id]["campaign_delivered"] += 1
        if row["opened_at"]:
            features[customer_id]["campaign_opened"] += 1
        if row["clicked_at"]:
            features[customer_id]["campaign_clicked"] += 1
    ...
    for row in read_rows(input_dir / "subscriptions.csv"):
        ...
        if row["status"] == "ACTIVE":
            features[customer_id]["active_subscriptions"] += 1
...
def summarize_segment(rows):
    ...
    delivered = sum(r["campaign_delivered"] for r in rows)
    opened = sum(r["campaign_opened"] for r in rows)
    clicked = sum(r["campaign_clicked"] for r in rows)
    active_subscriptions = sum(r["active_subscriptions"] for r in rows)
    ...
    return {
        ...
        "campaign_open_rate_pct": pct(opened, delivered),
        "campaign_click_rate_pct": pct(clicked, delivered),
        "active_subscription_rate_pct": pct(
            sum(1 for r in rows if r["active_subscriptions"] > 0),
            n,
        ),
        "mean_active_subscriptions": round(active_subscriptions / n, 3) if n else 0.0,
    }
...
def validate_campaign_engagement(features: Dict[str, dict]) -> dict:
    stable = summarize_segment(rows_for_segment(features, "stable"))
    declining = summarize_segment(rows_for_segment(features, "declining_engagement"))
    price = summarize_segment(rows_for_segment(features, "price_sensitive"))
    dormant = summarize_segment(rows_for_segment(features, "dormant"))

    price_click_lift = (
        price["campaign_click_rate_pct"] - stable["campaign_click_rate_pct"]
    )

    passed = (
        price_click_lift >= 10.0
        and declining["campaign_open_rate_pct"] < stable["campaign_open_rate_pct"]
        and dormant["campaign_open_rate_pct"] < declining["campaign_open_rate_pct"]
    )

    return {
        "stable_open_rate_pct": stable["campaign_open_rate_pct"],
        "declining_open_rate_pct": declining["campaign_open_rate_pct"],
        "dormant_open_rate_pct": dormant["campaign_open_rate_pct"],
        "stable_click_rate_pct": stable["campaign_click_rate_pct"],
        "price_sensitive_click_rate_pct": price["campaign_click_rate_pct"],
        "price_sensitive_click_lift_percentage_points": round(price_click_lift, 3),
        "minimum_click_lift_percentage_points": 10.0,
        "passed": passed,
    }


def validate_subscription_behavior(features: Dict[str, dict]) -> dict:
    stable = summarize_segment(rows_for_segment(features, "stable"))
    dormant = summarize_segment(rows_for_segment(features, "dormant"))
    declining = summarize_segment(rows_for_segment(features, "declining_engagement"))

    passed = (
        dormant["active_subscription_rate_pct"] < declining["active_subscription_rate_pct"]
        and declining["active_subscription_rate_pct"] < stable["active_subscription_rate_pct"]
    )

    return {
        "stable_active_subscription_rate_pct": stable["active_subscription_rate_pct"],
        "declining_active_subscription_rate_pct": declining["active_subscription_rate_pct"],
        "dormant_active_subscription_rate_pct": dormant["active_subscription_rate_pct"],
        "passed": passed,
    }
...
def main():
    ...
    tests = {
        ...
        "campaign_engagement": validate_campaign_engagement(features),
        "subscription_behavior": validate_subscription_behavior(features),
    }
